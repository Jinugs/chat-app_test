...
Положительные черты проекта:
   1. Быйстро смог понять логиу фронтенда и как нужно прописать сообщения, скроллы, а так же авто-обнолвения
   2. Деплой на сервер крайне простой, ожидалось больше проблем с пакетами Node
Нейтральные черты:
   1. Имеются некоторые проблемы с интерфейсом . Тут беда в том, что не на всех устройствах есть проблема и она мала, input-box для сообщений на некоторых устройствах сьезжает ниже, чем планировалось
   2. Не растягивается на весь экран чат, если сидеть с ПК, а не с смартфона (Я бы больше уделил этому времени, если бы не сроки)
Негативные черты:
   1. Крайне ограниченный функционал, как со стороны клиента, а со стороны сервер аего почти что нет (Будь это реальный проект с клиентом, то я бы выжимал с клиаента подробнейший функционал)
   2. Нету таймлайна с которого не загружались старые сообщение (из-за этого грузится вся БД разом)
   3. Отсутсвует авторизация, из-за чего IRT можно сменить ник и притворится другим человеком
Свою же работу я бы оценить 5\10. Можно былосправиться быйстрее и лучше
